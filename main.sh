...
function _boshness() {
  local main_options="-xx -e add-blob alias-env attach-disk back-up blobs cancel-task clean-up cloud-check cloud-config cpi-config create-env create-release delete-deployment delete-disk delete-env delete-release delete-snapshot delete-snapshots delete-stemcell delete-vm deploy deployment deployments disks environment environments envs errands events export-release finalize-release generate-job generate-package help ignore init-release inspect-release instances interpolate locks log-in log-out logs manifest recreate releases remove-blob reset-release restart run-errand runtime-config scp snapshots ssh start stemcells stop sync-blobs take-snapshot task tasks unignore update-cloud-config update-cpi-config update-resurrection update-runtime-config upload-blobs upload-release upload-stemcell variables vms"
    local no_options=""
